...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (window.localStorage.getItem('accesstoken')) {
    var settings = {
      url: `http://3.37.11.74/api/likes/${postId}/check`,
      method: 'GET',
      timeout: 0,
      headers: {
...
const getLikeCnt = () => {
  var likeCntSettings = {
    url: `http://3.37.11.74/api/likes/${postId}`,
    method: 'GET',
    timeout: 0,
  };
...
const getComments = (page) => {
  const commentSettings = {
    url: `http://3.37.11.74/api/post-comments/${postId}`,
    method: 'GET',
    timeout: 0,
    data: {
      page: page,
...
const like = () => {
  if (window.localStorage.getItem('accesstoken')) {
    var settings = {
      url: `http://3.37.11.74/api/likes/${postId}/like`,
      method: 'POST',
      timeout: 0,
      headers: {
        Authorization: window.localStorage.getItem('accesstoken'),
...
const unlike = () => {
  if (window.localStorage.getItem('accesstoken')) {
    var settings = {
      url: `http://3.37.11.74/api/likes/${postId}/unlike`,
      method: 'DELETE',
      timeout: 0,
      headers: {
        Authorization: window.localStorage.getItem('accesstoken'),
...
const updateComment = (postCommentId) => {
  ...
  var settings = {
    url: `http://3.37.11.74/api/post-comments/${postCommentId}`,
    method: 'PUT',
    timeout: 0,
    headers: {
      Authorization: window.localStorage.getItem('accesstoken'),

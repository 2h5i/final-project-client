...
document.addEventListener('DOMContentLoaded', function () {
  const recruitmentSettings = {
    url: `http://3.37.11.74/api/recruitments/${recruitmentId}`,
    method: 'GET',
    timeout: 0,
  };
  ...
  const commentSettings = {
    url: `http://3.37.11.74/api/recruitment-comments/${recruitmentId}`,
    method: 'GET',
    timeout: 0,
    data: {
      page: page - 1,
  ...
  if (window.localStorage.getItem('accesstoken')) {
    var settings = {
      url: `http://3.37.11.74/api/bookmarks/${recruitmentId}/check`,
      method: 'GET',
      timeout: 0,
      headers: {
...
const showUnBookmarkBtn = () => {
  ...
  const bookmark = () => {
    if (window.localStorage.getItem('accesstoken')) {
      var settings = {
        url: `http://3.37.11.74/api/bookmarks/${recruitmentId}/bookmark`,
        method: 'POST',
        timeout: 0,
        headers: {
          Authorization: window.localStorage.getItem('accesstoken'),
  ...
  const updateComment = (recruitmentCommentId) => {
    ...
    var settings = {
      url: `http://3.37.11.74/api/recruitment-comments/${recruitmentCommentId}`,
      method: 'PUT',
      timeout: 0,
      headers: {
        Authorization: window.localStorage.getItem('accesstoken'),
